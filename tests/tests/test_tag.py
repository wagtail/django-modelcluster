...
class TagTest(TestCase):
    def test_can_access_tags_on_unsaved_instance(self):
        ...
        self.assertTrue(
            [tag for tag in mission_burrito.tags.all() if tag.name == "mexican"]
        )
        ...
        mission_burrito.save()
        self.assertEqual(
            1, TaggedPlace.objects.filter(content_object_id=mission_burrito.id).count()
        )
        ...
        mission_burrito.save()
        self.assertEqual(
            0, TaggedPlace.objects.filter(content_object_id=mission_burrito.id).count()
        )
