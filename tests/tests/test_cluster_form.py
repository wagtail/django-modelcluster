...
class ClusterFormTest(TestCase):
    def test_cluster_form_with_no_formsets(self):
        ...
        self.assertInHTML(
            '<label for="id_albums-0-name">Name:</label>', form_html, count=0
        )
    ...
    def test_update_instance_with_unsaved_data(self):
        # Replicates saving a Wagtail page that has child objects that exist on the latest draft revision
        # but were never created in the database. At the point of saving the form, the child objects will
        # exist (without primary keys) both on the in-memory instance and in the form data. We need to
        # ensure that this does not result in duplicate child objects being created.
        # See https://github.com/wagtail/wagtail/issues/13332
    ...
    def test_widget_overrides(self):
        ...
        self.assertEqual(
            Textarea, type(form.formsets["members"].forms[0]["name"].field.widget)
        )
    ...
    def test_explicit_formset_dict(self):
        ...
        self.assertEqual(
            Textarea, type(form.formsets["albums"].forms[0]["name"].field.widget)
        )
    ...
    def test_without_kwarg_inheritance(self):
        ...
        self.assertInHTML(
            '<label for="id_members-0-name">Name!!!:</label>', form_html, count=0
        )
    ...
    def test_custom_formset_form(self):
        ...
        class BandForm(ClusterForm):
            class Meta:
                model = Band
                formsets = {"albums": {"fields": ["name"], "form": AlbumForm}}
                fields = ["name"]
    ...
    def test_alternative_formset_name(self):
        ...
        class BandForm(ClusterForm):
            class Meta:
                model = Band
                formsets = {"albums": {"fields": ["name"], "formset_name": "records"}}
                fields = ["name"]
...
class FormWithM2MTest(TestCase):
    ...
    def test_save_form_with_m2m(self):
        ...
        form = ArticleForm(
            {"title": "Updated test article", "authors": [self.charles_dickens.id]},
...
class NestedClusterFormTest(TestCase):
    ...
    def test_empty_nested_formsets(self):
        ...
        self.assertEqual(
            3, len(form.formsets["albums"].forms[0].formsets["songs"].forms)
        )
    ...
    def test_incoming_form_data(self):
        ...
        beatles = Band(
            ...
            albums=[
                Album(
                    name="Please Please Me",
                    songs=[Song(name="I Saw Her Standing There")],
                ),
            ],
        )
        form = BandForm(
    ...
    def test_sort_order_is_committed(self):
        ...
        self.assertEqual(
            "I Saw Her Standing There", beatles.albums.first().songs.all()[0].name
        )
